...
func add_unit(unit) -> void:
	...
	var lbl := unit.get_node_or_null("Label")
	if lbl:
...
func _try_move(unit, target: Vector2i) -> void:
	unit.grid_pos = target
	unit.position = grid_to_pixel(target)
	unit.mark_moved()
	_deselect()
	var adj = _adjacent_enemy(target)
	if adj != null:
...
func _execute_combat(attacker, defender) -> void:
	var pred = BattleCalculator.predict(attacker.data, defender.data, attacker.weapon_key, defender.weapon_key)
	_set_status("⚔ %s vs %s  命中%d%%  伤害%d" % [attacker.data.name, defender.data.name, pred["atk_hit"], pred["atk_damage"]])
...
func _refresh_unit_color(unit) -> void:
	var s := unit.get_node_or_null("Sprite")
	if s and unit.state == 3:
		s.color = Color(s.color.r * 0.5, s.color.g * 0.5, s.color.b * 0.5, 1.0)

func _restore_unit_color(unit) -> void:
	var s := unit.get_node_or_null("Sprite")
	if s:
...
func _check_defeat() -> void:
	if player_units.filter(func(u): return u.data.hp > 0).is_empty():
		_set_status("💀 全灭，战斗失败。")
		battle_lost.emit()

func _check_all_acted() -> void:
	if not player_units.any(func(u): return u.data.hp > 0 and u.can_act()):
...
func _start_enemy_turn() -> void:
	...
	for enemy in enemy_units:
		if enemy.data.hp <= 0:
			continue
		var action = EnemyAI.decide(enemy, player_units, _calc_move_range(enemy))
...
func _unit_at(pos: Vector2i, team: int):
	for u in (player_units + enemy_units):
		if u.grid_pos == pos and u.team == team and u.data.hp > 0:
			return u
	return null

func _adjacent_enemy(pos: Vector2i):
	for d in [Vector2i(1,0), Vector2i(-1,0), Vector2i(0,1), Vector2i(0,-1)]:
		var u = _unit_at(pos + d, 1)
		if u != null:
			return u
	return null

func _calc_move_range(unit) -> Array:
	var result: Array = []
	var visited := {}
	var queue := [{"pos": unit.grid_pos, "rem": unit.data.move}]
