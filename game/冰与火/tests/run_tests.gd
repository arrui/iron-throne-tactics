...
func _test_battle_predict_full() -> void:
	...
	var removed_attack_button_unit := Unit.new()
	removed_attack_button_unit.setup(_make_unit_data({"name": "点击攻击前被移除单位"}),
		0, Vector2i(5, 3))
	battle.get_node("UnitLayer").add_child(removed_attack_button_unit)
	battle.player_units.append(removed_attack_button_unit)
	battle.selected_unit = removed_attack_button_unit
	battle.player_state = battle.PlayerState.UNIT_MOVED
	battle.attack_tiles.assign([defender.grid_pos])
	battle._show_action_menu(removed_attack_button_unit.grid_pos, true)
	battle.player_units.erase(removed_attack_button_unit)
	removed_attack_button_unit.free()
	attack_button.pressed.emit()
...
func _test_item_system() -> void:
	...
	var removed_item_user := Unit.new()
	removed_item_user.setup(_make_unit_data({
		"name": "道具使用前被移除单位",
		"items": [{"name": "急救药", "type": "heal", "heal_amount": 10, "uses": 1}],
	}), 0, Vector2i(4, 4))
	battle.get_node("UnitLayer").add_child(removed_item_user)
	battle.player_units.append(removed_item_user)
	battle.selected_unit = removed_item_user
	battle.player_state = battle.PlayerState.UNIT_MOVED
	battle._show_items_panel(removed_item_user)
	var removed_user_panel := battle._active_items_panel as PanelContainer
	var removed_user_button := removed_user_panel.get_child(0).get_child(1) as Button
	battle.player_units.erase(removed_item_user)
	removed_item_user.free()
	removed_user_button.pressed.emit()
	await process_frame
	_assert(not is_instance_valid(battle.selected_unit) \
			and battle.player_state == battle.PlayerState.IDLE \
	...
	var removed_action_menu_unit := Unit.new()
	removed_action_menu_unit.setup(_make_unit_data({
		"name": "显示菜单前被移除单位",
		"items": [{"name": "急救药", "type": "heal", "heal_amount": 10, "uses": 1}],
	}), 0, Vector2i(4, 7))
	battle.get_node("UnitLayer").add_child(removed_action_menu_unit)
	battle.player_units.append(removed_action_menu_unit)
	battle.selected_unit = removed_action_menu_unit
	battle.player_state = battle.PlayerState.UNIT_MOVED
	var removed_action_menu_pos := removed_action_menu_unit.grid_pos
	battle.player_units.erase(removed_action_menu_unit)
	removed_action_menu_unit.free()
	battle._show_action_menu(removed_action_menu_pos, false)
	await process_frame
	_assert(not is_instance_valid(battle.selected_unit) \
			and battle.player_state == battle.PlayerState.IDLE \
			and not action_menu.visible,
		"行动菜单渲染遇到已释放单位时保持关闭并清理选择态")
...
func _test_settings_menu() -> void:
	...
	_assert_eq(settings_close_btn.get_theme_color("font_color"), BattleChromeThemeClass.TEXT_PRIMARY,
		"保存并返回按钮使用中性文字色")
	_assert(settings_title != null and settings_title.get_theme_font("font") != null,
		"设置菜单标题显式应用 CJK 字体")
...
func _test_unit_state_machine() -> void:
	...
	var removed_in_place_unit := Unit.new()
	removed_in_place_unit.setup(_make_unit_data({"name": "原地菜单前被移除单位"}),
		0, Vector2i(5, 5))
	battle.get_node("UnitLayer").add_child(removed_in_place_unit)
	battle.player_units.append(removed_in_place_unit)
	battle.selected_unit = removed_in_place_unit
	battle.player_state = battle.PlayerState.UNIT_SELECTED
	battle.move_range.clear()
	battle.move_range.append(removed_in_place_unit.grid_pos)
	battle.attack_tiles.clear()
	battle.attack_tiles.append(Vector2i(6, 5))
	battle.player_units.erase(removed_in_place_unit)
	removed_in_place_unit.free()
	battle._open_in_place_menu()
	_assert(not is_instance_valid(battle.selected_unit) \
			and battle.player_state == battle.PlayerState.IDLE \
			and battle.move_range.is_empty() \
			and battle.attack_tiles.is_empty() \
			and not action_menu.visible,
		"原地行动菜单遇到已释放选中单位时清理选择与行动范围")
